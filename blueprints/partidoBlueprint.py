...
@partido_blueprint.route("/resultado/all", methods=['GET'])
def get_partido():
    response = partido_controller.index()
    return response, 200


@partido_blueprint.route("/resultado/<string:id_>", methods=['GET'])
def get_partido_by_id(id_):
    response = partido_controller.show(id_)
    return response, 200


@partido_blueprint.route("/resultado/insert", methods=['POST'])
def insert_partido():
...
@partido_blueprint.route("resultado/update/<string:id_>", methods=['PATCH'])
...
@partido_blueprint.route("resultado/delete/<string:id_>", methods=['DELETE'])
